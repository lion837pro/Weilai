...
public class Pose{
     ...
     */
    public static final Pose kZero = new Pose();
    ...
    public void setX(double newX){
        m_translation.setX(newX);
    }

    public void setY(double newY){
        m_translation.setY(newY);
    }
    ...
    public Transform minus(Pose other) {
        final Pose pose = this.relativeTo(other);
    ...
    public Pose times(double scalar) {
        return new Pose(m_translation.times(scalar), m_rotation.times(scalar));
    }
     ...
     * @return The transformed pose.
     */
    public Pose rotateBy(Rotation other) {
        return new Pose(m_translation.rotateBy(other), m_rotation.rotateBy(other));
    }
    ...
    public Pose relativeTo(Pose other) {
        Transform transform = new Transform(other, this);
     ...
     */
    public Pose rotateAround(Translation point, Rotation rot) {
        return new Pose(m_translation.rotateAround(point, rot), m_rotation.rotateBy(rot));
    }
    ...
    public Pose exp(Twist twist) {
        ...
        Transform transform =
                new Transform(
                        new Translation(dx * s - dy * c, dx * c + dy * s),
    ...
    public Twist log(Pose end) {
        final Pose transform = end.relativeTo(this);
     ...
     */
    public Pose nearest(List<Pose> poses) {
        return Collections.min(
                poses,
                Comparator.comparing(
                                (Pose other) -> this.getTranslation().getDistance(other.getTranslation()))
                        .thenComparing(
                                (Pose other) ->
    ...
    /**
     * Returns a double array representation of the pose.
     *
     * @return A double array representation of the pose.
     */
    public double[] toArray() {
        return  new double[]{m_translation.getX(), m_translation.getY(), m_rotation.getRadians()};
    }

    public com.pedropathing.geometry.Pose toPedroPose(){
        return new com.pedropathing.geometry.Pose(m_translation.getX(), m_translation.getY(), m_rotation.getRadians());
    }
