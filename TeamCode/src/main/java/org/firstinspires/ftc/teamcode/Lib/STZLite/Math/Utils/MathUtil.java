...
public final class MathUtil {
     ...
     */
    public static Translation slewRateLimit(
            Translation current, Translation next, double dt, double maxVelocity) {
        if (maxVelocity < 0) {
            return next;
        }
        Translation diff = next.minus(current);
    ...
    public static double coterminalDeg(double angle){
    ...
    public static double coterminalRad(double angle){
    ...
    public static double minAngleDeg(double ang_from, double ang_to) {
        if (Math.abs(ang_to - ang_from) < coterminalDeg(ang_to - ang_from)) {
            return ang_to - ang_from;
        }
        else {
            return coterminalDeg(ang_to - ang_from);
        }
    }
    public static double MinAngleRad(double ang_from, double ang_to) {
        if (Math.abs(ang_to - ang_from) < coterminalRad(ang_to - ang_from)) {
            return ang_to - ang_from;
        }
        else {
            return coterminalRad(ang_to - ang_from);
        }
    }

    public static double atan3Rad(double x, double y, double threshold, double defaultValue) {
    ...
    public static double atan3Deg(double x, double y, double threshold, double defaultValue) {
        return Units.radiansToDegrees(atan3Rad(x, y, threshold, defaultValue));
    }
