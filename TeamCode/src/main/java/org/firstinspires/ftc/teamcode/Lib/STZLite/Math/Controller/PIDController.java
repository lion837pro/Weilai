...
/** Implements a PID control loop. (Versión standalone) */
public class PIDController {
    ...
    public PIDController(ProfileGains.PIDGains gains){
        this(gains.kP(), gains.kI(), gains.kD());
    }
     ...
     */
    public void setGains(ProfileGains.PIDGains gains){
        m_kp = gains.kP();
        m_ki = gains.kI();
        m_kd = gains.kD();
    }
     ...
     * @return gains
     */
    public ProfileGains.PIDGains getGains(){
        return new ProfileGains.PIDGains(m_kp, m_ki, m_kd);
    }
    ...
    public Output calculate(double measurement, double dt) {
        ...
        if (dt > 0.0) {
            ...
            // Usa el 'dt' real para el Integral
